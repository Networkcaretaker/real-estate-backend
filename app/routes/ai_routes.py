...
# Property Assistant
@ai_bp.route('/analyze-property', methods=['POST', 'OPTIONS'])
@cross_origin(
    origins=['http://localhost:5173', 'http://127.0.0.1:5173'],
    methods=['POST', 'OPTIONS'],
    allow_headers=['Content-Type', 'Authorization']
...
def analyze_property():
    try:
        logger.info("Received property AI analysis request")
        ...
        # Process AI analysis
        ai_service = AIService()
        versions = ai_service.analyze_property_content(
            data['property_id'],
            data['image_id'],
            data['versions']
        )
        ...
        logger.error("Property AI analysis error", error=str(e), exc_info=True)
        return jsonify({
            'status': 'error',
            'message': str(e)
        }), 500